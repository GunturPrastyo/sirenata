@props([
    'meta',   // array: current_page, last_page, from, to, total
])

@php
    $currentPage = $meta['current_page'] ?? 1;
    $lastPage    = $meta['last_page'] ?? 1;
    $from        = $meta['from'] ?? 0;
    $to          = $meta['to'] ?? 0;
    $total       = $meta['total'] ?? 0;

    // Generate halaman yang ditampilkan — max 3 di tengah + ellipsis
    $pages = [];
    if ($lastPage <= 7) {
        $pages = range(1, $lastPage);
    } else {
        $pages = [1, 2, 3];
        if ($currentPage > 4) {
            $pages[] = '...';
        }
        for ($i = max(4, $currentPage - 1); $i <= min($lastPage - 3, $currentPage + 1); $i++) {
            $pages[] = $i;
        }
        if ($currentPage < $lastPage - 3) {
            $pages[] = '...';
        }
        $pages = array_merge($pages, [$lastPage - 2, $lastPage - 1, $lastPage]);
        $pages = array_unique($pages);
    }

    // Helper untuk generate URL web (bukan URL API)
    $pageUrl = fn($page) => request()->fullUrlWithQuery(['page' => $page]);
@endphp

@if($lastPage > 1)
<div class="flex items-center justify-between w-full h-16 px-3 border-t border-neutral-200 mt-4">
    <p class="pl-2 text-sm text-gray-700">
        Showing
        <span class="font-medium">{{ $from }}</span>
        to
        <span class="font-medium">{{ $to }}</span>
        of
        <span class="font-medium">{{ $total }}</span>
        results
    </p>

    <nav>
        <ul class="flex items-center text-sm leading-tight bg-white border divide-x rounded h-9 text-neutral-500 divide-neutral-200 border-neutral-200">

            {{-- Previous --}}
            <li class="h-full">
                @if($currentPage > 1)
                    <a href="{{ $pageUrl($currentPage - 1) }}"
                        class="relative inline-flex items-center h-full px-3 ml-0 rounded-l group hover:text-neutral-900">
                        <span>Previous</span>
                    </a>
                @else
                    <span class="relative inline-flex items-center h-full px-3 ml-0 rounded-l text-neutral-300 cursor-not-allowed">
                        Previous
                    </span>
                @endif
            </li>

            {{-- Page numbers --}}
            @foreach($pages as $page)
                @if($page === '...')
                    <li class="hidden h-full md:block">
                        <div class="relative inline-flex items-center h-full px-2.5 group">
                            <span>...</span>
                        </div>
                    </li>
                @elseif($page === $currentPage)
                    <li class="hidden h-full md:block">
                        <span class="relative inline-flex items-center h-full px-3 text-neutral-900 group bg-gray-50 cursor-default">
                            <span>{{ $page }}</span>
                            <span class="box-content absolute bottom-0 left-0 w-full h-px -mx-px translate-y-px border-l border-r bg-neutral-900 border-neutral-900"></span>
                        </span>
                    </li>
                @else
                    <li class="hidden h-full md:block">
                        <a href="{{ $pageUrl($page) }}"
                            class="relative inline-flex items-center h-full px-3 group hover:text-neutral-900">
                            <span>{{ $page }}</span>
                            <span class="box-content absolute bottom-0 w-0 h-px -mx-px duration-200 ease-out translate-y-px border-transparent bg-neutral-900 group-hover:border-l group-hover:border-r group-hover:border-neutral-900 left-1/2 group-hover:left-0 group-hover:w-full"></span>
                        </a>
                    </li>
                @endif
            @endforeach

            {{-- Next --}}
            <li class="h-full">
                @if($currentPage < $lastPage)
                    <a href="{{ $pageUrl($currentPage + 1) }}"
                        class="relative inline-flex items-center h-full px-3 rounded-r group hover:text-neutral-900">
                        <span>Next</span>
                    </a>
                @else
                    <span class="relative inline-flex items-center h-full px-3 rounded-r text-neutral-300 cursor-not-allowed">
                        Next
                    </span>
                @endif
            </li>

        </ul>
    </nav>
</div>
@endif