@props(['name', 'title' => 'Modal Title', 'maxWidth' => 'sm:max-w-lg', 'id' => null])

<div x-data="{
    show: false,
    modalId: '{{ $id ?? $name }}'
}" x-show="show"
    x-on:open-modal.window="if ($event.detail === '{{ $name }}' || $event.detail === modalId) { show = true }"
    x-on:close-modal.window="if ($event.detail === '{{ $name }}' || $event.detail === modalId) { show = false }"
    x-on:keydown.escape.window="show = false" style="display: none;" {{ $attributes->merge(['class' => '']) }}>

    <template x-teleport="body">
        <div x-show="show"
            class="fixed top-0 left-0 z-[99] p-2 md:p-0 flex items-center justify-center w-screen h-screen" x-cloak>
