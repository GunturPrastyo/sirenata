<x-dashboard::layouts.dashboard title="Rencana Tenaga Kerja Kabupaten/Kota Update">
    <div class="p-2 sm:p-6">
        <x-breadcrumb :items="[
            ['label' => 'Rekapitulasi Rencana Tenaga Kerja Kab/Kota', 'url' => route('admin-kab-kota.rtkd.index')],
            ['label' => 'Edit Rencana Tenaga Kerja Kab/Kota']
        ]" />
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-red-600 mt-1">
                            ...
                            {{ $rtkd->approver?->name ?? 'Admin Pusat/Admin Province' }}.
        ...
        <div class="bg-white rounded-md shadow-sm p-10">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <!-- Left Column: Form -->
                <div>
                    <form action="{{ route('admin-kab-kota.rtkd.update', $rtkd->id) }}" method="POST" id="uploadForm"
                        class="space-y-8" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <!-- Nama -->
                        <x-form.input name="name" label="Nama" required value="{{ old('name', $rtkd->name) }}" placeholder="Masukkan nama dokumen RTKN" />
                        ...
                        {{-- <div>
                            ...
                            <select name="status" class="w-full rounded-md border-gray-300">
                                @foreach (\Modules\RTK\Enums\RTKStatus::cases() as $status)
                                    <option value="{{ $status->value }}" @selected(old('status', $rtkd->status?->value ?? $rtkd->status) == $status->value)>
                                        {{ $status->value }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <x-form.input type="number" name="start_date" label="Berlaku Dari Tahun" required value="{{ old('start_date', $rtkd->start_date) }}" placeholder="2025" />
                            <x-form.input type="number" name="end_date" label="Sampai Tahun" required value="{{ old('end_date', $rtkd->end_date) }}" placeholder="2030" />
                        </div>
