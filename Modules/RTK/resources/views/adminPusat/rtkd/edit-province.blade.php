<x-dashboard::layouts.dashboard title="Rencana Tenaga Kerja Daerah Provinsi Edit">
    <div class="p-2 sm:p-6">
        ...
        <div class="">
            <x-button type="button" x-data @click="$dispatch('open-modal', 'preview-document')" class="mb-3" variant="white">
        ...
        <div class="bg-white rounded-md shadow-sm p-10">
            <div class="grid grid-cols-1 lg:grid-cols-1 gap-8">
                <!-- Left Column: Form -->
                <div>
                    <form action="{{ route('admin-pusat.rtkd.update-province', ['provinceCode' => $provinceCode, 'rtkdp' => $rtkdp->id]) }}" method="POST" id="uploadForm" class="space-y-8" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <!-- Nama -->
                        <x-form.input name="name" label="Nama" :value="$rtkdp->name" disabled placeholder="Masukkan nama dokumen Rencana Tenaga Kerja Daerah Provinsi" />
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <x-form.input type="number" name="start_date" label="Berlaku Dari Tahun" :value="$rtkdp->start_date" disabled placeholder="2025" />
                            <x-form.input type="number" name="end_date" label="Sampai Tahun" :value="$rtkdp->end_date" disabled placeholder="2030" />
                        </div>

                        <!-- Action Buttons -->
                        <div class="flex gap-4 pt-4">
                            <x-button :href="route('admin-pusat.rtkd.show-province', $provinceCode)" variant="secondary" size="lg" class="flex-1">
                                Batal
                            </x-button>
