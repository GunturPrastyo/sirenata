<x-dashboard::layouts.dashboard title="Manajemen Lembaga (Pusat)">
    <div class="p-2 sm:p-6">
        <!-- Breadcrumb Navigation -->
        <x-breadcrumb :items="[['label' => 'Lembaga']]" />

        <x-dashboard::filter-card
            title="Daftar Lembaga"
            :total="$institutions->total() . ' Lembaga'"
            :resetUrl="route('super-admin.lembaga.index')">

            <x-slot name="actions">
                <x-button type="button" x-data @click="$dispatch('open-modal', 'create-lembaga')" size="sm" icon="fas fa-plus">
                    <span class="hidden sm:inline">Tambah Lembaga</span>
            ...
            <x-table.table plain>
                <thead>
                    <tr>
                        ...
                        <x-table.th>Nama Lembaga / Kementerian</x-table.th>
                    ...
                    @forelse($institutions as $key => $instansi)
                        <tr class="hover:bg-slate-50 transition">
                            ...
                            <x-table.td align="center">
                                <x-table.action>
                                    <li>
                                        <button type="button" x-data @click="$dispatch('open-modal', 'edit-lembaga-{{ $instansi->id }}')"
                                            class="inline-flex items-center w-full p-2 hover:bg-slate-100 rounded text-amber-600 cursor-pointer text-left">Ubah</button>
                                    </li>
                                    <li>
                                        <form action="{{ route('super-admin.lembaga.toggle-status', $instansi->id) }}" method="POST" class="w-full">
                                            @csrf
                                            @method('PATCH')
                                    ...
                                    <li>
                                        <div class="inline-flex items-center w-full p-2 hover:bg-slate-100 rounded">
                                            <x-modal-delete :id="'delete-lembaga-' . $instansi->id" message="Apakah Anda yakin ingin menghapus lembaga ini?"
                                                :item-name="$instansi->name" buttonText="Hapus" buttonClass="w-full text-left text-red-600 outline-none cursor-pointer" :route="route('super-admin.lembaga.destroy', $instansi->id)" />
                                        </div>
                                    </li>
                                </x-table.action>
                            </x-table.td>
                        ...
                        <x-modal name="edit-lembaga-{{ $instansi->id }}" title="Edit Lembaga">
                            <form action="{{ route('super-admin.lembaga.update', $instansi->id) }}" method="POST" class="space-y-4 text-left">
                                @csrf
                                @method('PUT')

                                <x-form.input name="name" id="name-{{ $instansi->id }}" label="Nama Lembaga (Pusat)" value="{{ old('name', $instansi->name) }}" required placeholder="Contoh: Kementerian Kesehatan" />
                                ...
                                <p class="text-xs text-gray-500 -mt-2 ml-6">Jika tidak aktif, lembaga ini tidak akan muncul di pilihan registrasi user.</p>
    ...
    <x-modal name="create-lembaga" title="Tambah Lembaga Baru">
        <form action="{{ route('super-admin.lembaga.store') }}" method="POST" class="space-y-4">
            @csrf

            <x-form.input name="name" id="name" label="Nama Lembaga (Pusat)" value="{{ old('name') }}" required placeholder="Contoh: Kementerian Kesehatan" />
