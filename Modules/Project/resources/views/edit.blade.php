        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 sm:p-8 max-w-2xl mx-auto">
            ...
            <form action="{{ route($routePrefix . 'update', $project->id) }}" method="POST"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    <x-form.input type="date" name="startDate" label="Tanggal Mulai" required value="{{ old('startDate', $project->start_date ? \Carbon\Carbon::parse($project->start_date)->format('Y-m-d') : '') }}" />
                    <x-form.input type="date" name="endDate" label="Tanggal Selesai" required value="{{ old('endDate', $project->end_date ? \Carbon\Carbon::parse($project->end_date)->format('Y-m-d') : '') }}" />
                </div>

                <x-form.input type="number" name="duration" label="Durasi (Bulan)" placeholder="Contoh: 12" value="{{ old('duration', $project->duration) }}" />
                ...
                <x-form.select id="teamMembers" name="teamMembers[]" label="Anggota Tim (Opsional)" multiple>
                    @foreach($users as $user)
                        <option value="{{ $user->id }}" @selected(in_array($user->id, old('teamMembers') ?? (is_array($project->team_members) ? $project->team_members : json_decode($project->team_members, true) ?? [])))>
                            {{ $user->name }}
                        </option>
                    @endforeach
                </x-form.select>
