<x-dashboard::layouts.dashboard title="Tambah Proyek Pusat - E-Learning">
    @push('styles')
    ...
    <div class="p-2 sm:p-6" x-data="projectCreateForm({{ json_encode($users) }})" x-init="watchPrerequisites()">
        <x-breadcrumb :items="[['label' => 'Proyek', 'url' => route($routePrefix . 'index')], ['label' => 'Tambah Proyek Pusat']]" />
        ...
        <div class="bg-white rounded-lg border border-slate-100 shadow-sm p-6 sm:p-8 max-w-full mx-auto">
            <div class="mb-6 sm:mb-8 border-b border-slate-100 pb-5 sm:pb-6">
                <h1 class="text-xl sm:text-2xl font-extrabold text-slate-900 tracking-tight">Tambah Proyek Pusat Baru</h1>
                <p class="text-sm text-slate-500 mt-1">Lengkapi data proyek, filter prasyarat kursus, serta tentukan Ketua dan Anggota Tim.</p>
                ...
                <div class="border-t border-slate-200 pt-6 mt-6 space-y-6">
                    ...
                    <div class="bg-slate-50 border border-slate-200 rounded-xl p-4 sm:p-5">
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <span class="text-xs text-slate-500 font-medium">
                                Filter Aktif: <strong class="text-indigo-600" x-text="selectedCourses.length + ' Kursus'"></strong>
                            </span>
                        </div>
                        <p class="text-xs text-slate-500 mb-3">
                            Hanya pengguna yang <b>lulus 100%</b> pada seluruh kursus yang dipilih di bawah ini yang akan tampil pada opsi Ketua & Anggota Tim.
                        </p>
                        
                        <!-- Searchbox Prasyarat (Debounce 300ms) -->
                        <div class="mb-2" x-show="allCourses.length > 5">
                            <input type="text" 
                                   x-model.debounce.300ms="searchCourse" 
                        ...
                        <!-- Checkbox List Kursus Prasyarat (Menggunakan x-model murni) -->
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5 max-h-48 overflow-y-auto p-2 bg-white rounded-lg border border-slate-200">
                            <template x-for="course in filteredCourses" :key="course.id">
                                <label class="flex items-center gap-2.5 p-2 rounded-md hover:bg-slate-50 cursor-pointer border border-transparent hover:border-slate-200 transition text-xs font-medium text-slate-700">
                                    <input type="checkbox" 
                            ...
                            <div x-show="filteredCourses.length === 0" class="col-span-full text-xs text-slate-400 text-center py-2">
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <select id="teamLeader" name="teamLeader" required class="w-full rounded-lg border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm p-2.5">
                            ...
                            <p x-show="filteredUsers.length === 0" class="text-xs text-amber-600 mt-1">
                                <i class="fas fa-exclamation-triangle mr-1"></i> Tidak ada pengguna yang memenuhi seluruh prasyarat kursus yang dipilih.
                            </p>
                        </div>
                        ...
                        <div class="relative" @click.outside="isMemberDropdownOpen = false">
                            <div class="flex items-center justify-between mb-1">
                                <label class="block text-sm font-medium text-slate-700">Anggota Tim (Opsional)</label>
                                <span class="text-xs text-slate-500" x-text="selectedMembers.length + ' Anggota Terpilih'"></span>
                            </div>

                            <!-- Badges Anggota Terpilih -->
                            <div class="flex flex-wrap gap-1.5 my-2" x-show="selectedMembers.length > 0">
                                <template x-for="user in selectedMemberObjects" :key="user.id">
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-indigo-50 text-indigo-700 border border-indigo-200">
                                        ...
                                        <button type="button" @click="removeMember(user.id)" class="text-indigo-400 hover:text-indigo-700 focus:outline-none">
                            ...
                            <button type="button" 
                                    ...
                                    class="w-full flex items-center justify-between bg-white border border-slate-300 rounded-lg p-2.5 text-sm text-slate-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                <span x-text="selectedMembers.length > 0 ? selectedMembers.length + ' Anggota Terpilih' : '-- Pilih Anggota Tim --'" 
                                      :class="selectedMembers.length > 0 ? 'text-slate-900 font-medium' : 'text-slate-400'"></span>
                                <i class="fas fa-chevron-down text-slate-400 transition-transform duration-200" :class="{ 'rotate-180': isMemberDropdownOpen }"></i>
                            </button>
                            ...
                            <div x-show="isMemberDropdownOpen" 
                                 ...
                                 class="absolute z-20 left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg p-3 space-y-2">
                                
                                <!-- Searchbar di Dalam Dropdown (Debounce 300ms) -->
                                <div class="relative">
                                    <input type="text" 
                                           x-model.debounce.300ms="searchMember" 
                                           placeholder="Cari nama anggota tim..." 
                                           class="w-full text-xs rounded-md border-slate-300 focus:ring-indigo-500 focus:border-indigo-500 p-2 pr-8">
                                ...
                                <div class="max-h-48 overflow-y-auto space-y-1 p-1">
                                    <template x-for="user in filteredMembersList" :key="user.id">
                                        <label class="flex items-center gap-2.5 p-2 rounded-md hover:bg-slate-50 cursor-pointer text-xs font-medium text-slate-700 transition">
                                            <input type="checkbox" 
                                                   name="teamMembers[]" 
                                    ...
                                    <div x-show="filteredMembersList.length === 0" class="text-xs text-slate-400 text-center py-3">
                ...
                Alpine.data('projectCreateForm', (allUsers) => ({
                    ...
                    get filteredUsers() {
                        ...
                        return this.allUsers.filter(user => {
                            const userCourses = (user.completed_courses || []).map(id => String(id));
                    ...
                    removeMember(userId) {
                        this.selectedMembers = this.selectedMembers.filter(id => String(id) !== String(userId));
                    },

                    watchPrerequisites() {
                        this.$watch('selectedCourses', () => {
                            const validIds = new Set(this.filteredUsers.map(u => String(u.id)));
                            this.selectedMembers = this.selectedMembers.filter(id => validIds.has(String(id)));
