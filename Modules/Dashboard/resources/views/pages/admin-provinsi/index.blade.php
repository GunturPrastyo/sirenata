<x-dashboard::layouts.dashboard title="Dashboard Admin Provinsi">
    <div class="p-4 sm:p-6 lg:p-6 max-w-full mx-auto space-y-6 sm:space-y-8 bg-slate-50/50 min-h-screen">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div class="lg:col-span-7 xl:col-span-8 bg-white rounded-md shadow-sm border border-slate-200 overflow-hidden flex flex-col">
                    ...
                    <div class="w-full sm:w-auto sm:max-w-xs shrink-0 flex items-center gap-2">
                        <select id="rtkYearFilter" onchange="fetchRtkProvinsiData(this.value)"
                            ...
                            @foreach ($rtkYearsOptions as $y)
                                <option value="{{ $y }}" {{ (string) $y === (string) $selectedRtkYear ? 'selected' : '' }}>
                                    Mulai {{ $y }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="p-5 sm:p-6 flex-1">
                    ...
                    <div class="max-h-[500px] overflow-y-auto custom-scrollbar pr-2 {{ $rtkMasaAktifPerKabKota->count() > 0 ? '' : 'hidden' }}">
                    ...
                    <div id="rtkCombinedEmptyState"
                        class="bg-slate-50 rounded-lg p-10 text-center border border-dashed border-slate-200 my-4 {{ $rtkMasaAktifPerKabKota->count() > 0 ? 'hidden' : '' }}">
                        ...
                        <p class="text-xs text-slate-500 mt-1">Belum ada penyusunan dokumen RTK Kab/Kota yang tercatat.</p>
                    </div>
                </div>
            </div>
                ...
                <div class="p-4 flex-1 flex flex-col justify-between">
                    ...
                    <div id="pendingScrollContainer"
                        ...
                        <div id="pendingApprovalList" class="space-y-3">
                            ...
                            @empty
                                <div class="bg-slate-50 rounded-lg p-8 text-center border border-dashed border-slate-200 my-2">
                                    <div class="w-10 h-10 bg-white rounded-full flex items-center justify-center mx-auto mb-2 shadow-sm text-slate-400">
        ...
        <div class="bg-white rounded-md shadow-sm border border-slate-200 overflow-hidden">
            <div class="px-5 sm:px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    <div>
                        ...
                        <p class="text-[11px] sm:text-xs text-slate-500">Jumlah pengguna terdaftar berdasarkan Kabupaten/Kota</p>
                    </div>
                </div>

                <div class="w-full sm:w-auto sm:max-w-xs shrink-0">
                    <select id="sdmYearFilter" onchange="fetchSdmProvinsiData(this.value)"
                        class="w-full text-sm border-slate-200 rounded-lg focus:ring-[#13416B] focus:border-[#13416B] text-ellipsis overflow-hidden cursor-pointer bg-slate-50">
            ...
            <div class="p-5 sm:p-6">
                ...
                <div id="sdmListContainer"
                    class="space-y-6 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar {{ $sdmPerKabKota->count() > 0 ? '' : 'hidden' }}">
                    <!-- Di-render via JavaScript agar dinamis -->
                </div>

                <!-- EMPTY STATE -->
                <div id="sdmEmptyState"
                    class="bg-slate-50 rounded-lg p-10 text-center border border-dashed border-slate-200 my-4 {{ $sdmPerKabKota->count() > 0 ? 'hidden' : '' }}">
                    <div class="w-14 h-14 bg-white rounded-full flex items-center justify-center mx-auto mb-3 shadow-sm text-slate-400">
            ...
            document.addEventListener('DOMContentLoaded', function() {
                    ...
                    fetch(`{{ route('admin-province.dashboard') }}?rtk_year=${year}`, {
                        ...
                        .then(data => {
                            ...
                            if (!rtkData || rtkData.length === 0) {
                                ...
                                const chartColorPalette = ['#13416B', '#547996', '#8BB1CC'];
                                const newBarColors = newFloatingData.map((_, i) => chartColorPalette[i % chartColorPalette.length]);
                                ...
                                if (window.rtkCombinedChartInstance) {
                                    ...
                                    window.rtkCombinedChartInstance.data.datasets[0].backgroundColor = newBarColors;
                ...
                if (pendingContainer) {
                    pendingContainer.addEventListener('scroll', function() {
                        ...
                        if (pendingContainer.scrollTop + pendingContainer.clientHeight >= pendingContainer.scrollHeight - 30) {
                            loadMorePendingData();
                        }
                    });
                }
                ...
                function loadMorePendingData() {
                    ...
                    fetch(`{{ route('admin-province.dashboard') }}?pending_page=${nextPage}`, {
                            headers: { 'X-Requested-With': 'XMLHttpRequest' }
                        })
                        .then(res => res.json())
                        .then(res => {
