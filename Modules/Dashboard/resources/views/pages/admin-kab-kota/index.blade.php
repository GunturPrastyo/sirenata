    ...
    <div class="p-4 sm:p-6 lg:p-8 max-w-full mx-auto space-y-6 sm:space-y-8 bg-slate-50/50 min-h-screen">
        ...
        <div class="flex flex-col gap-4">
            ...
            @if (!$rtkStatusInfo['hasAcuan'])
                <div
                    class="bg-sky-50 border border-sky-200 rounded-lg p-4 sm:p-5 shadow-sm flex items-start justify-between gap-4">
                    <div class="flex items-start gap-3.5">
                        <div
                            class="w-10 h-10 rounded-lg bg-sky-100 text-[#13416B] flex items-center justify-center shrink-0">
                        ...
                        <div>
                            ...
                            <p class="text-xs sm:text-sm text-sky-800 leading-relaxed">
                                Wilayah Anda belum memiliki dokumen yang ditandai sebagai <strong>RTK Acuan
                                    (is_active)</strong>.
                                Sesuai ketentuan, <strong>hanya dokumen acuan yang akan diverifikasi dan disetujui oleh
                                    Admin Pusat</strong>.
                            </p>
                        </div>
                    </div>
            ...
            @if ($rtkStatusInfo['isRejected'])
                <div
                    class="bg-rose-50 border border-rose-200 rounded-lg p-4 sm:p-5 shadow-sm flex items-start justify-between gap-4">
                    <div class="flex items-start gap-3.5">
                        <div
                            class="w-10 h-10 rounded-lg bg-rose-100 text-rose-600 flex items-center justify-center shrink-0">
                        ...
                        <div>
                            ...
                            <p class="text-xs sm:text-sm text-rose-700 leading-relaxed">
                                Dokumen acuan <strong>{{ $rtkAcuan->name }}</strong> dikembalikan oleh verifikator pusat
                                karena memerlukan perbaikan.
            ...
            @if ($rtkStatusInfo['isExpiringSoon'] && $rtkStatusInfo['isValid'])
                <div
                    class="bg-amber-50 border border-amber-200 rounded-lg p-4 sm:p-5 shadow-sm flex items-start justify-between gap-4">
                    <div class="flex items-start gap-3.5">
                        <div
                            class="w-10 h-10 rounded-lg bg-amber-100 text-amber-600 flex items-center justify-center shrink-0">
                        ...
                        <div>
                            ...
                            <p class="text-xs sm:text-sm text-amber-700 leading-relaxed">
                                Dokumen acuan <strong>{{ $rtkAcuan->name }}</strong> menyisakan
                                <strong>{{ $rtkStatusInfo['sisaWaktuTeks'] }}</strong>.
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
            ...
            <div
                class="lg:col-span-7 bg-white rounded-md shadow-sm border border-slate-200 overflow-hidden flex flex-col justify-between h-full">
                <div>
                    <!-- Header -->
                    <div class="px-5 sm:px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-[11px] text-slate-500">Hanya dokumen berstatus Acuan yang diverifikasi &
                                disetujui</p>
                        </div>
                        <a href="{{ route('admin-kab-kota.rtkd.index') }}"
                    ...
                    <div class="p-5 sm:p-6 space-y-5">
                        @if ($rtkAcuan)
                            ...
                            <div class="bg-slate-50/80 rounded-xl p-4 border border-slate-200/80 space-y-3.5">
                                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                    <div>
                                        <div class="flex items-center gap-2">
                                            <span
                                                class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-[#13416B] text-white">
                                                <i class="fas fa-check-circle mr-1"></i> RTK Acuan Aktif
                                            </span>
                                            <span class="text-xs text-slate-400">Periode {{ $rtkAcuan->start_date }} -
                                                {{ $rtkAcuan->end_date }}</span>
                                        </div>
                                        <h3 class="text-sm font-bold text-slate-800 mt-1">{{ $rtkAcuan->name }}</h3>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span
                                            class="px-2.5 py-1 rounded text-[11px] font-semibold {{ $rtkStatusInfo['statusBadgeColor'] }}">
                                ...
                                <div
                                    class="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-3 border-t border-slate-200/70 text-xs">
                                    <div>
                                        <span class="text-slate-400 text-[11px] block">Masa Berlaku</span>
                                        <span
                                            class="font-bold {{ $rtkStatusInfo['sisaWaktuColor'] }}">{{ $rtkStatusInfo['sisaWaktuTeks'] }}</span>
                                    </div>
                                    <div>
                                        <span class="text-slate-400 text-[11px] block">Status Dokumen</span>
                                        <span
                                            class="font-semibold text-slate-700">{{ $rtkAcuan->status_document?->label() ?? 'Belum Berlaku' }}</span>
                                    </div>
                                    <div>
                                        <span class="text-slate-400 text-[11px] block">Pembaruan Terakhir</span>
                                        <span
                                            class="font-medium text-slate-600">{{ $rtkAcuan->updated_at ? $rtkAcuan->updated_at->diffForHumans() : '-' }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-white border border-slate-200 rounded-xl p-4 space-y-3">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 text-xs">
                                    ...
                                    <div
                                        class="p-3 rounded-md border {{ $rtkAcuan->is_active ? 'border-emerald-200 bg-emerald-50/50' : 'border-slate-200 bg-slate-50' }}">
                                        <div class="flex items-center gap-2 mb-1">
                                            <span
                                                class="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold {{ $rtkAcuan->is_active ? 'bg-emerald-600 text-white' : 'bg-slate-300 text-slate-600' }}">1</span>
                                ...
                                @else border-slate-200 bg-slate-50 @endif">
                                        ...
                                        <p class="text-[11px] text-slate-500">
                                            @if ($rtkStatusInfo['isApproved'])
                                                Verifikasi Disetujui
                                            @elseif($rtkStatusInfo['isPending'])
                                                Menunggu Review
                                            @elseif($rtkStatusInfo['isRejected'])
                                    ...
                                    <div
                                        class="p-3 rounded-md border {{ $rtkStatusInfo['isValid'] ? 'border-emerald-200 bg-emerald-50/50' : 'border-slate-200 bg-slate-50' }}">
                                        <div class="flex items-center gap-2 mb-1">
                                            <span
                                                class="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold {{ $rtkStatusInfo['isValid'] ? 'bg-emerald-600 text-white' : 'bg-slate-300 text-slate-600' }}">3</span>
                        ...
                        @else
                            ...
                            <div
                                class="flex flex-col items-center justify-center text-center py-10 bg-slate-50 rounded-lg border border-dashed border-slate-200">
                                <div
                                    class="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm text-amber-500 mb-3">
                                    <i class="fas fa-file-signature text-xl"></i>
                                </div>
                                <h4 class="text-sm font-bold text-slate-700">Belum Ada RTKD Acuan</h4>
                                <p class="text-xs text-slate-500 mt-1 max-w-sm">
                                    Admin Pusat hanya memproses dokumen yang ditandai sebagai <strong>RTK Acuan
                                        (is_active)</strong>. Silakan ajukan dokumen atau pilih dokumen yang telah ada
                                    untuk diaktifkan sebagai acuan.
                                </p>
                                <a href="{{ route('admin-kab-kota.rtkd.create') }}"
                ...
                <!-- Footer Action Buttons Kiri -->
                <div
                    class="px-5 sm:px-6 py-3.5 bg-slate-50/80 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2">
                    <span class="text-xs text-slate-500">
                        <i class="fas fa-shield-alt text-[#13416B] mr-1"></i> Hanya dokumen acuan aktif yang disetujui
                        admin provinsi
                    </span>
                    <div class="flex items-center gap-2">
                        <a href="{{ route('admin-kab-kota.rtkd.index') }}"
            ...
            <div
                ...
                <div>
                    ...
                    <div class="p-5 sm:p-6 space-y-4">
                        @if ($totalProjects > 0)
                            @php
                                $avgProgress = round($averageProgress ?? 0);
                                $dashOffset = round(($avgProgress / 100) * 251.2, 1);
                            @endphp

                            <!-- Circular Progress Visual -->
                            <div
                                class="flex items-center gap-5 p-3.5 bg-slate-50/80 rounded-lg border border-slate-200/80">
                                <div class="relative w-20 h-20 shrink-0 flex items-center justify-center">
                                    <svg class="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                                        ...
                                        @if ($avgProgress > 0)
                                            <circle cx="50" cy="50" r="40" stroke="#f59e0b"
                                                stroke-width="10" fill="none" stroke-linecap="round"
                                                stroke-dasharray="{{ $dashOffset }} 251.2" />
                                        @endif
                                    </svg>
                                    <div class="absolute text-center">
                                        <span
                                            class="text-base font-extrabold text-slate-800">{{ $avgProgress }}%</span>
                                    </div>
                                </div>
                                <div class="min-w-0 flex-1">
                                    <h4 class="text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">Status
                                        Pengerjaan</h4>
                                    <div class="flex items-baseline gap-2">
                                        <span
                                            class="text-2xl font-black text-amber-500">{{ $onProgressProjects }}</span>
                                        <span class="text-xs text-slate-500 font-medium">berjalan dari
                                ...
                                <div class="relative">
                                    ...
                                    <div class="max-h-[172px] overflow-hidden space-y-2.5">
                                        @foreach ($recentProjects as $p)
                                            <div
                                                class="p-2.5 rounded-lg border border-slate-100 bg-white hover:bg-slate-50 transition-colors">
                                                <div class="flex justify-between items-start gap-2 mb-1.5">
                                                    ...
                                                    <span
                                                        class="text-[10px] font-bold px-1.5 py-0.5 rounded {{ $p->progress >= 100 ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700' }} shrink-0">
                                                        {{ $p->progress }}%
                                                    </span>
                                                </div>
                                                <div
                                                    class="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden mb-1">
                                                    <div class="bg-[#13416B] h-full rounded-full transition-all duration-500"
                                                        style="width: {{ $p->progress }}%"></div>
                                                </div>
                                                <div
                                                    class="flex items-center justify-between text-[10px] text-slate-400">
                                                    <span><i
                                                            class="far fa-user mr-1"></i>{{ $p->leader?->name ?? 'Tim Daerah' }}</span>
                                                    @if ($p->prerequisiteCourse)
                                                        <span class="text-[#547996] font-medium"
                                                            title="Prasyarat Kursus LMS: {{ $p->prerequisiteCourse->name }}">
                                    ...
                                    <!-- Efek Gradient Fade Out di Bawah -->
                                    <div
                        ...
                        @else
                            ...
                            <div
                                class="flex flex-col items-center justify-center text-center py-10 bg-slate-50 rounded-lg border border-dashed border-slate-200">
                                <div
                                    class="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm text-slate-400 mb-3">
                                    <i class="fas fa-clipboard-list text-xl"></i>
                                </div>
                                <h4 class="text-sm font-bold text-slate-700">Belum Ada Proyek Daerah</h4>
                                <p class="text-xs text-slate-400 mt-1 max-w-xs">Wilayah ini belum memiliki proyek atau
                                    kegiatan perencanaan.</p>
                            </div>
                        @endif
                    </div>
                </div>

                <!-- Footer Action Buttons Kanan -->
                <div
                    class="px-5 sm:px-6 py-3.5 bg-slate-50/80 border-t border-slate-100 flex items-center justify-end">
        ...
        <div class="bg-white rounded-md shadow-sm border border-slate-200 overflow-hidden">
            <div
                ...
                <div>
                    ...
                    <p class="text-[11px] sm:text-xs text-slate-500">Statistik pertumbuhan peserta dalam blok 5 tahun
                        terakhir</p>
                </div>
                ...
                <div class="w-full sm:w-auto sm:max-w-xs shrink-0 flex items-center gap-2">
                    <select id="yearFilter" onchange="filterDataYear(this.value)"
                        ...
                        @foreach ($years as $y)
                            <option value="{{ $y }}"
                                {{ (string) $y === (string) $selectedYear ? 'selected' : '' }}>
                                Tahun Berakhir {{ $y }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="bg-white rounded-md shadow-sm border border-slate-200 overflow-hidden">
            <div class="px-5 sm:px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-[11px] sm:text-xs text-slate-500">Peringkat kursus yang diikuti dan sebaran gender
                    </p>
                </div>
            </div>
            ...
            <div class="p-5 sm:p-6 grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
                ...
                <div class="lg:col-span-2 border-b lg:border-b-0 lg:border-r border-slate-100 pb-6 lg:pb-0 lg:pr-8">
                    ...
                    <div id="courseEmptyState"
                        ...
                        <p class="text-xs text-slate-400 mt-1">Belum ada pengguna di wilayah ini yang mendaftar pada
                            modul pelatihan.</p>
                    </div>
                </div>
