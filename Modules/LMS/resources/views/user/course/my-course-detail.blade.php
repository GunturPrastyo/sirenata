<x-dashboard::layouts.dashboard
    ...
    <div class="p-2 sm:p-6 max-w-full mx-auto">
        ...
        <div
            class="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 sm:p-6 lg:p-8 mb-6 sm:mb-8 flex flex-col md:flex-row gap-6 lg:gap-8 items-start transition-all">

            {{-- Bagian Kiri: Thumbnail --}}
            <div
                class="w-full md:w-1/3 lg:w-1/4 shrink-0 rounded-xl overflow-hidden bg-slate-100 aspect-video md:aspect-square lg:aspect-[4/3] relative border border-slate-100">
            ...
            <div class="flex-1 flex flex-col w-full h-full">
                ...
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <span
                        class="px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider {{ data_get($course, 'status') === 'completed' ? 'text-emerald-700 bg-emerald-100 border border-emerald-200' : 'text-amber-700 bg-amber-100 border border-amber-200' }} rounded-md flex items-center gap-1">
                        <i
                            class="fas {{ data_get($course, 'status') === 'completed' ? 'fa-check-circle' : 'fa-clock' }}"></i>
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 sm:p-6">
                    <div class="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
                        ...
                        <span
                            class="text-xs text-slate-500 font-bold bg-slate-100 px-3 py-1.5 rounded-md border border-slate-200">
                            ...
                            <div x-data="{ id: 'section-{{ $index }}', locked: {{ $isLocked ? 'true' : 'false' }} }"
                                ...
                                <button @click="if(!locked) activeAccordion = (activeAccordion == id ? '' : id)"
                                    ...
                                    <div class="flex items-start gap-4">
                                        @if ($isLocked)
                                            <span
                                                class="flex items-center justify-center w-7 h-7 rounded-full bg-slate-200 text-slate-400 text-xs mt-0.5 shrink-0 border border-slate-300">
                                                <i class="fas fa-lock"></i>
                                            </span>
                                        @elseif($isSectionCompleted)
                                            <span
                                                class="flex items-center justify-center w-7 h-7 rounded-full bg-emerald-50 text-emerald-600 text-xs mt-0.5 shrink-0 border border-emerald-200">
                                                <i class="fas fa-check"></i>
                                            </span>
                                        @else
                                            <span
                                                class="flex items-center justify-center w-7 h-7 rounded-full bg-slate-100 text-slate-600 text-xs font-bold mt-0.5 shrink-0 border border-slate-200">
                                                {{ $index + 1 }}
                                            </span>
                                        @endif
                                        ...
                                        <div>
                                            <h3
                                                class="font-bold {{ $isLocked ? 'text-slate-500' : 'text-slate-800' }} text-sm sm:text-base leading-snug">
                                                {{ $sectionName }}
                                            </h3>
                                            <div class="flex items-center gap-2 mt-1">
                                                ...
                                                @else
                                                    <span
                                                        class="text-xs font-semibold {{ $isSectionCompleted ? 'text-[#13416B]' : 'text-amber-600' }}">
                                ...
                                <div x-show="activeAccordion == id && !locked" x-collapse x-cloak>
                                    <div class="p-4 pt-2 border-t border-slate-100 bg-slate-50/30 space-y-3">
                                        ...
                                        @forelse ($sectionContentsRaw as $content)
                                            ...
                                            @endphp
                                            <div
                                                class="flex flex-col sm:flex-row sm:items-center justify-between p-3.5 rounded-xl border border-slate-200 bg-white hover:border-[#13416B]/40 hover:shadow-sm transition-all duration-200 gap-4">
                                                <div class="flex items-center gap-4">
                                                    @if (!empty($videoUrlRaw))
                                                        <span
                                                            class="flex items-center justify-center w-10 h-10 rounded-xl bg-blue-50 text-blue-600 shrink-0 border border-blue-100">
                                        ...
                                        @if ($postTestBab)
                                            <div class="mt-4 pt-4 border-t border-slate-200/80">
                                                {{-- Kartu Post Test: Jika belum selesai pakai Biru Tua & teks putih. Jika selesai pakai gaya netral --}}
                                                <div
                                                    class="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border {{ $isPostTestBabCompleted ? 'border-[#13416B]/20 bg-[#13416B]/5' : 'bg-[#13416B] border-[#13416B]' }} hover:shadow-sm transition-all gap-4">
                                                    <div class="flex items-center gap-4">
                                                        <span
                                                            class="flex items-center justify-center w-10 h-10 rounded-xl {{ $isPostTestBabCompleted ? 'bg-[#13416B] text-white border border-[#13416B]' : 'bg-white/20 text-white border border-white/30' }} shrink-0">
                                                            <i class="fas fa-clipboard-check text-lg"></i>
                                                        </span>
                                                        <div>
                                                            <p
                                                                class="font-bold text-sm leading-tight {{ $isPostTestBabCompleted ? 'text-slate-800' : 'text-white' }}">
                                                                {{ data_get($postTestBab, 'title', 'Post Test: ' . $sectionName) }}
                                                            </p>
                                                            @if (data_get($postTestBab, 'description'))
                                                                <p
                                                                    class="text-xs mt-1 line-clamp-2 {{ $isPostTestBabCompleted ? 'text-slate-500' : 'text-blue-100' }}">
                                                    ...
                                                    {{-- Tombol Kerjakan Evaluasi (Kuning, teks putih) / Lihat Hasil --}}
                                                    <a href="{{ route('user.course.test.show', ['slug' => $courseSlug, 'postTestId' => $postTestBab->id]) }}"
                                                        class="sm:self-center px-5 py-2.5 {{ $isPostTestBabCompleted ? 'bg-white border border-[#13416B]/30 text-[#13416B] hover:bg-[#13416B]/10' : 'bg-amber-400 hover:bg-amber-500 text-gray-800 shadow-sm font-bold' }} rounded-xl text-xs transition-all text-center shrink-0 flex items-center justify-center gap-2">
                                                        <i
                ...
                @if ($evaluasiAkhir)
                    <div
                        ...
                        <div
                            class="px-6 py-5 {{ $isEvaluasiAkhirLocked ? 'bg-slate-50 border-b border-slate-200' : 'bg-gradient-to-r from-[#13416B] to-[#0f3354] text-white' }} flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <div class="flex items-center gap-4">
                                <div
                                ...
                                <div>
                                    <p
                                        class="text-xs font-bold uppercase tracking-wider {{ $isEvaluasiAkhirLocked ? 'text-slate-400' : 'text-blue-100' }} mb-1">
                                        Tahap Akhir
                                    </p>
                                    <h3
                                        class="text-lg font-extrabold {{ $isEvaluasiAkhirLocked ? 'text-slate-800' : 'text-white' }} leading-tight">
                            ...
                            @if ($isEvaluasiAkhirLocked)
                                <span
                                    class="px-4 py-2 text-xs font-bold text-slate-500 bg-white border border-slate-200 rounded-xl text-center shadow-sm">
                        ...
                        <div class="p-6 bg-white">
                            <p
                                class="text-sm {{ $isEvaluasiAkhirLocked ? 'text-slate-500' : 'text-slate-600' }} leading-relaxed">
                            ...
                            <div class="mt-5 flex flex-wrap items-center gap-3">
                                <span
                                    class="text-xs font-semibold text-slate-600 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200 flex items-center gap-1.5">
            ...
            <div class="space-y-6 lg:sticky lg:top-24 lg:self-start">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 sm:p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div
                            class="p-3.5 bg-slate-50 rounded-xl border border-slate-100 flex flex-col justify-between">
                            <span
                                class="text-[10px] uppercase font-bold text-slate-500 mb-2 flex items-center gap-1"><i
                                    class="fas fa-info-circle"></i> Status</span>
                            <span
                                class="inline-flex items-center justify-center px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider {{ data_get($course, 'status') === 'completed' ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' : 'bg-amber-100 text-amber-700 border border-amber-200' }}">
                        ...
                        <div
                            class="p-3.5 bg-slate-50 rounded-xl border border-slate-100 flex flex-col justify-between">
                            <span
                                class="text-[10px] uppercase font-bold text-slate-500 mb-1 flex items-center gap-1"><i
                                    class="fas fa-layer-group"></i> Modul</span>
                            <span
                                class="text-sm font-extrabold text-slate-800">{{ count(data_get($course, 'sections', [])) }}</span>
                        </div>
                        <div
                            class="p-3.5 bg-slate-50 rounded-xl border border-slate-100 flex flex-col justify-between">
                            <span
                                class="text-[10px] uppercase font-bold text-slate-500 mb-1 flex items-center gap-1"><i
                                    class="fas fa-file-alt"></i> Materi</span>
                        ...
                        <div
                            ...
                            <span
                                class="text-sm font-extrabold text-[#13416B]">{{ data_get($course, 'completed_count', 0) }}
                        ...
                        <div class="py-6 flex flex-col items-center justify-center text-center px-2">
                            ...
                            @if ($remaining > 0)
                                <p class="text-xs text-slate-500 leading-relaxed">
                                    Selesaikan <span class="font-bold text-slate-800">{{ $remaining }}
                                        materi</span> dan evaluasi akhir untuk membuka sertifikat kelulusan.
                                </p>
                            @else
                                <p class="text-xs text-slate-500 leading-relaxed">
