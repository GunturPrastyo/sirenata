    ...
    <div class="p-2 sm:p-6 max-w-full mx-auto">
        ...
        @php
            ...
            foreach(data_get($course, 'sections', []) as $section) {
                $postTestBab = \Modules\LMS\Models\PostTest::where('course_section_id', data_get($section, 'id', data_get($section, 'section_id')))->first();
                if ($postTestBab) {
                    $totalTests++;
            ...
            $evaluasiAkhir = $courseId ? \Modules\LMS\Models\PostTest::where('course_id', $courseId)->whereNull('course_section_id')->first() : null;
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            <div class="lg:col-span-2 space-y-6">
                                    ...
                                    <div class="p-4 pt-2 border-t border-slate-100 bg-slate-50/30 space-y-3">
                                        ...
                                        @if ($postTestBab)
                                            <div class="mt-4 pt-4 border-t border-slate-200/80">
                                                <div class="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border {{ $isPostTestBabCompleted ? 'border-[#13416B]/20 bg-[#13416B]/5' : 'bg-[#13416B] border-[#13416B]' }} hover:shadow-sm transition-all gap-4">
                                                    <div class="flex items-center gap-4">
                                                        <span class="flex items-center justify-center w-10 h-10 rounded-xl {{ $isPostTestBabCompleted ? 'bg-[#13416B] text-white border border-[#13416B]' : 'bg-white/20 text-white border border-white/30' }} shrink-0">
                                                            <i class="fas fa-clipboard-check text-lg"></i>
                                                        </span>
                                                        <div>
                                                            <p class="font-bold text-sm leading-tight {{ $isPostTestBabCompleted ? 'text-slate-800' : 'text-white' }}">
                                                                {{ data_get($postTestBab, 'title', 'Post Test: ' . $sectionName) }}
                                                            </p>
                                                            @if (data_get($postTestBab, 'description'))
                                                                <p class="text-xs mt-1 line-clamp-2 {{ $isPostTestBabCompleted ? 'text-slate-500' : 'text-blue-100' }}">
                ...
                @if ($evaluasiAkhir)
                    <div class="bg-white rounded-lg shadow-sm border border-gray-50 overflow-hidden transition-all relative">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 sm:p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="p-3.5 bg-slate-50 rounded-xl border border-slate-100 flex flex-col justify-between">
                            ...
                            <span class="inline-flex items-center justify-center px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider {{ $isFullyCompleted ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' : 'bg-amber-100 text-amber-700 border border-amber-200' }}">
                        ...
                        <div class="p-3.5 bg-slate-50 rounded-xl border border-slate-100 flex flex-col justify-between">
                            ...
                            <span class="text-sm font-extrabold text-[#13416B]">{{ $completedItems }}
                                <span class="text-xs text-slate-400 font-medium">/ {{ $totalItems }}</span></span>
                        </div>
                    </div>
                </div>
                    ...
                    @if ($isFullyCompleted)
                        @if (!empty(data_get($course, 'certificate_file')))
                            <div class="space-y-5">
                                ...
                                <a href="{{ data_get($course, 'certificate_file') }}" target="_blank" download class="w-full bg-[#13416B] hover:bg-[#0f3354] text-white py-3 rounded-xl font-bold transition-all shadow-sm flex items-center justify-center gap-2 text-sm">
                        ...
                        @else
                            <div class="space-y-5">
                                ...
                                <form action="{{ route('user.course.my-course.generate-certificate', ['slug' => request()->route('slug')]) }}" method="POST" x-data="{ loading: false }" @submit="loading = true">
                                    @csrf
                                    <button type="submit" :disabled="loading" class="w-full bg-[#13416B] hover:bg-[#0f3354] text-white py-3 rounded-xl font-bold transition-all shadow-sm flex items-center justify-center gap-2 text-sm disabled:opacity-70 disabled:cursor-not-allowed">
