<x-dashboard::layouts.dashboard title="Edit Post Test & Soal">
    ...
    <div class="p-2 sm:p-6" x-data="postTestEditForm()">
        ...
        <form action="{{ route('admin-pusat.management-course.post-tests.update', $postTest->id) }}" method="POST" class="max-w-full mx-auto">
            @csrf
            @method('PUT')
            <!-- Hidden Inputs -->
            ...
            <div class="flex flex-col lg:flex-row gap-6 items-start">
                ...
                <div class="flex-1 w-full space-y-6">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden scroll-mt-24" id="section-pengaturan">
                        ...
                        <div class="p-6 space-y-4">
                            <div>
                                ...
                                <input type="text" name="title" required value="{{ old('title', $postTest->title) }}" placeholder="Contoh: Ujian Evaluasi Pemahaman Bab 1" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
                            </div>

                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-slate-700 mb-1">Passing Score (KKM) <span class="text-red-500">*</span></label>
                                    <input type="number" name="passing_score" required min="0" max="100" value="{{ old('passing_score', $postTest->passing_score) }}" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-slate-700 mb-1">Durasi (Menit) <span class="text-red-500">*</span></label>
                                    <input type="number" name="duration" required min="1" value="{{ old('duration', $postTest->duration) }}" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
                                </div>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Instruksi Pengerjaan (Opsional)</label>
                                <textarea name="description" rows="2" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm">{{ old('description', $postTest->description) }}</textarea>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div x-data="{
                                                initEditor() {
                                                    ...
                                                    quill.root.innerHTML = q.question; // Pre-fill dengan data existing
                                                ...
                                                <div class="w-full flex-1">
                                                    <div x-data="{
                                                            initChoiceEditor() {
                                                                ...
                                                                quill.root.innerHTML = q.choices[cIndex]; // Pre-fill dengan data existing
                                                                quill.on('text-change', () => { 
                    ...
                    <div class="flex justify-end gap-3 pt-4 pb-12">
                        ...
                        <button type="submit" class="px-6 py-2.5 text-sm font-bold text-white bg-amber-600 rounded-xl hover:bg-amber-700 transition shadow-sm flex items-center gap-2">
                            <i class="fas fa-save"></i> Perbarui Post Test & Soal
                        </button>
                    </div>
                </div>
                ...
                <div class="hidden lg:block lg:w-72 shrink-0 lg:sticky lg:top-24 mb-0">
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
                        <div class="flex items-center gap-3 mb-4 pb-3 border-b border-slate-100">
                            <!-- Icon Background Sirenata -->
                            <div class="w-8 h-8 flex items-center justify-center bg-[#13416B] text-white rounded-lg shadow-sm border border-[#0f3354] shrink-0">
                                <i class="fas fa-th-large text-sm"></i>
                            </div>
                            <h3 class="text-sm font-bold text-slate-800">Navigasi Soal Cepat</h3>
                        </div>
        ...
        <div x-show="showMobileNav" style="display: none;" class="fixed inset-0 z-[9999] lg:hidden" x-cloak>
            ...
            <div x-show="showMobileNav" 
                ...
                <div class="p-4 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white rounded-t-2xl z-10">
                    ...
                    <button type="button" @click="showMobileNav = false" class="w-8 h-8 flex items-center justify-center rounded-full bg-slate-100 text-slate-500 hover:bg-slate-200 focus:outline-none shrink-0">
    ...
    <script>
        ...
        // DEFINISI STATE ALPINEJS (DENGAN INJEKSI DATA EXISTING)
        function postTestEditForm() {
            return {
                showMobileNav: false, // State untuk Bottom Sheet
                questions: @json($formattedQuestions), // Injeksi data dari backend
                addQuestion() {
                    this.questions.push({
                        question: '',
                        choices: ['', '', '', ''],
