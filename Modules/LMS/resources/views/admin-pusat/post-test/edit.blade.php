        ...
        <nav class="flex mb-4 sm:mb-6" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 sm:space-x-3 flex-wrap">
                ...
                <li>
                    <div class="flex items-center">
                        <i class="fas fa-chevron-right text-slate-400 text-xs mx-1"></i>
                        <span class="ml-1 text-sm font-medium text-slate-500 md:ml-2">Edit Post Test</span>
                    </div>
                </li>
            </ol>
        </nav>

        <form action="{{ route('admin-pusat.management-course.post-tests.update', $postTest->id) }}" method="POST" class="max-w-full mx-auto space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <input type="text" name="title" required value="{{ old('title', $postTest->title) }}" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative group">
                        ...
                        <div class="p-6 space-y-4">
                            <div>
                                ...
                                <textarea x-model="q.question" :name="'questions['+qIndex+'][question]'" rows="3" required class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"></textarea>
                            </div>
                                ...
                                <template x-for="(choice, cIndex) in q.choices" :key="cIndex">
                                    <div class="flex items-start gap-3 bg-slate-50/50 p-3 rounded-lg border border-slate-100">
                                        ...
                                        <div class="w-full flex-1">
                                            <input type="text" x-model="q.choices[cIndex]" :name="'questions['+qIndex+'][choices]['+cIndex+']'" required class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm bg-white">
                                        </div>
            ...
            <div class="flex justify-end gap-3 pt-4 pb-12">
                ...
                <button type="submit" class="px-6 py-2 text-sm font-medium text-white bg-amber-600 rounded-lg hover:bg-amber-700 transition shadow-sm flex items-center gap-2">
