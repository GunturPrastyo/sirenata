    ...
    <div class="p-2 sm:p-6">
        ...
        <div class="bg-white rounded-lg shadow-sm p-4 sm:p-8 max-w-3xl border border-gray-100">
            <div class="mb-4 sm:mb-6">
                <h1 class="text-lg sm:text-2xl font-bold text-gray-900">Tambah Materi Perpustakaan</h1>
                <p class="text-gray-600 mt-1 text-sm sm:text-base">Lengkapi form di bawah untuk menambah buku, dokumen,
                    atau video baru.</p>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="library_type_id" name="library_type_id" required
                            ...
                            @foreach($libraryTypes as $type)
                                <option value="{{ $type->id }}" {{ old('library_type_id') == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            Gambar Sampul <span class="text-xs text-gray-500 font-normal">(Opsional, max 2MB)</span>
                        </label>
                        <input type="file" name="cover_image" accept="image/*"
                            class="w-full border border-gray-300 rounded-lg p-1 text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 transition-colors">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            File Dokumen <span class="text-xs text-gray-500 font-normal">(Opsional, max 20MB)</span>
                        </label>
                        <input type="file" name="file_path" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx"
                            class="w-full border border-gray-300 rounded-lg p-1 text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-gray-50 file:text-gray-700 hover:file:bg-gray-100 transition-colors">
                    </div>

                    <div class="sm:col-span-2">
                        <label for="external_link" class="block text-sm font-medium text-gray-700 mb-2">
                            Link Eksternal <span class="text-xs text-gray-500 font-normal">(Contoh: Link YouTube,
                                opsional)</span>
                        </label>
